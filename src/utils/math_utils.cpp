...
namespace math_utils {
    ...
    float calc_angle(float *centroid_1, float *centroid_2, float *vec, size_t dim) {
        float dot = 0.0f, norm1 = 0.0f, norm2 = 0.0f;
        for (size_t i = 0; i < dim; i++) {
            float v1 = centroid_1[i] - vec[i];
            float v2 = centroid_2[i] - vec[i];
            dot += v1 * v2;
            norm1 += v1 * v1;
            norm2 += v2 * v2;
        }
...
#pragma omp parallel for schedule(static, 8192)
    ...
    void rotate_data_randomly(float *data, size_t num_points, size_t dim, float *rot_mat, float *&new_mat,
        ...
        cblas_sgemm(CblasRowMajor, CblasNoTrans, transpose, num_points, dim, dim, 1.0, data, dim, rot_mat, dim, 0,
                    new_mat,
                    dim);
    ...
    void compute_closest_centers(float *data, size_t num_points, size_t dim, float *pivot_data, size_t num_centers,
                                 size_t k, uint32_t *closest_centers_ivf, float *beta) {
        if (k > num_centers) {
...
#pragma omp for schedule(dynamic, 1)
            for (size_t cur_blk = 0; cur_blk < N_BLOCKS; cur_blk++) {
                ...
                cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, num_pts_blk, num_centers, dim, -2.0f, block_data,
                            dim,
                    ...
                    for (size_t i = 0; i < num_pts_blk; i++) {
                        ...
                        if (beta != nullptr) {
                            ...
                            for (size_t j = 0; j < k; j++) {
                                ...
                                temp[j] = top_k_queue.top().piv_dist;
                                top_k_queue.pop();
                            }
                            beta[block_start + i] = temp[1] / temp[0];
                        } else {
                            for (size_t j = 0; j < k; j++) {
                                output[j] = (uint32_t) top_k_queue.top().piv_id;
                                top_k_queue.pop();
...
namespace kmeans {
    ...
    float elkan_iter(float *data, size_t num_points, size_t dim, float *centers, size_t num_centers,
                     std::vector<size_t> *closest_docs, uint32_t *&closest_center, float *lower_bound,
                     float *upper_bound,
        ...
        for (int64_t chunk = 0; chunk < static_cast<int64_t>(nchunks); ++chunk) {
            for (size_t d = chunk * CHUNK_SIZE; d < num_points && d < (chunk + 1) * CHUNK_SIZE; ++d) {
                residuals[chunk * BUF_PAD] +=
                        math_utils::calc_distance(data + (d * dim), centers + (size_t) closest_center[d] * (size_t) dim,
                                                  dim);
            }
        }
...
#pragma omp parallel for schedule(static, 8192)
        ...
        for (size_t i = 0; i < max_reps; ++i) {
            ...
            residual = elkan_iter(data, num_points, dim, centers, num_centers, closest_docs, closest_center,
                                  lower_bound,
